...
%
% This function reads the fixed-noise EKF validation products generated by
% plotProductionOptimizationResults and creates conference-paper heatmaps for
% the overall-best network in each production case.
%
% Figures:
%   1) Per-RSO RMS position error versus N_s for information and coverage.
%      A logarithmic color scale is used so isolated EKF-divergence cases do
%      not hide the behavior of the remaining RSOs.
%   2) Per-RSO accepted-measurement availability versus N_s for information
%      and coverage. Availability is normalized by N_s times the number of
%      tracking epochs so network sizes can be compared directly.
%
% A compact CSV/console summary is also produced with the median RMS position
% error, worst-RSO RMS position error, and minimum/median measurement
% availability for every objective/network-size case.
%
% Usage:
...
summaryFile = fullfile(tableDirectory,"ekf_per_rso_heatmap_summary.csv");
writetable(summaryTable,summaryFile);

fprintf("\n============================================================\n");
fprintf("Per-RSO EKF heatmap summary\n");
fprintf("============================================================\n");
disp(summaryTable);

%% Figure 1: RMS position-error heatmap
...
for objectiveIndex = 1:numberOfObjectives
    ...
    styleHeatmapAxes(ax,style,config.networkSizes,numberOfObjects);
...
positionColorbar.TickLabels = compose("10^{%d}",positionTicks);
positionColorbar.TickLabelInterpreter = "tex";
...
%% Figure 2: normalized measurement-availability heatmap
...
availabilityOutputFile = fullfile(config.outputDirectory, ...
...
heatmapInfo = struct();
heatmapInfo.version = "per_rso_ekf_heatmaps_v1";
...
fprintf("\nGenerated per-RSO EKF heatmaps:\n");
fprintf("  %s\n",positionOutputFile);
...
function styleHeatmapAxes(ax,style,networkSizes,numberOfObjects)
ax.YDir = "reverse";
...
switch lower(string(objectiveMode))
    case "information"
        titleText = "Information objective";
    case "coverage"
        titleText = "Coverage objective";
    otherwise
