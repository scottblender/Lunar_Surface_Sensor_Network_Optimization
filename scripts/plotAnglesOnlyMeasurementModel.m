...
% Keep the projected-range label slightly left of center so it does not sit
% directly on the projected LOS vector.
labelPosition = sensor + 0.60*projectionLength*[cos(alpha);sin(alpha)] + 0.24*normal;
...
scatter(axRa,projectionEnd(1),projectionEnd(2),125,"p", ...
...
% Shift the horizontal projected-range label left so it clears the LOS.
text(axDec,sensor(1)+0.50*projectionLength,sensor(2)-0.24,"\rho_{xy,k}", ...
    "Interpreter","tex","Color",projectionColor,"FontName",fontName, ...
    "FontSize",axisLabelFontSize,"FontWeight","bold", ...
    "BackgroundColor",labelBackgroundColor,"Margin",style.labelMargin, ...
    "HorizontalAlignment","center");
...
% Offset the LOS-range label toward the upper-left side of the vector so the
% white label backing does not cover the magenta LOS shaft.
losLabelPosition = sensor + 0.56*losVector + 0.34*losNormal;
...
% Use a larger RSO marker so the endpoint remains distinct at manuscript size.
scatter(axDec,rsoPosition(1),rsoPosition(2),170,"p", ...
    "MarkerFaceColor",losColor,"MarkerEdgeColor",backgroundColor,"LineWidth",1.2);
text(axDec,rsoPosition(1)+0.18,rsoPosition(2)+0.04,"RSO", ...
    "Color",textColor,"FontName",fontName,"FontSize",annotationFontSize, ...
    "FontWeight","bold","BackgroundColor",labelBackgroundColor, ...
    "Margin",style.labelMargin, ...
