...
%% Equal-count Jacobian chunks
% Keep the original full 2x6 RA/Dec Jacobians, including the zero direct-
% velocity columns, but partition the retained global candidate list into six
% approximately equal-sized MAT files. Objective evaluations use MATFILE
% partial indexing through the global -> chunk/local index map.
...
if isfolder(chunkDirectory)
    existingChunkFiles = dir(fullfile(chunkDirectory,"measurement_jacobian_chunk_*.mat"));
    assert(isempty(existingChunkFiles), ...
        ["Existing Jacobian chunk files were found in:\n%s\n" ...
...
if config.chunking.enabled
    fprintf("Jacobian storage: full 2x6, %d equal-count chunks\n", ...
        config.chunking.numberOfChunks);
