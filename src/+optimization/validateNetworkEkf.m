...
if isfield(database.visibility,"candidateChunks") && ...
    ...
    selectedFilteredAvailability = ...
        optimization.loadChunkedCandidateData( ...
            database,sensorIndices,"filteredAvailability");
else
    selectedFilteredAvailability = ...
        database.visibility.filteredAvailability(sensorIndices,:,:);
end

assert(size(selectedFilteredAvailability,1) == numberOfSelectedSensors && ...
    size(selectedFilteredAvailability,2) == numberOfTimes && ...
    size(selectedFilteredAvailability,3) == numberOfObjects, ...
    "validateNetworkEkf:InvalidAvailabilityDimensions", ...
    "Selected filtered availability has unexpected dimensions.");
...
for objectIndex = 1:numberOfObjects
    ...
    objectAvailability = reshape( ...
        selectedFilteredAvailability(:,:,objectIndex), ...
