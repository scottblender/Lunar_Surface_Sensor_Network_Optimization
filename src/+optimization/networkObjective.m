...
if calculateInformation
    ...
    % The production database stores only the three nonzero position
    % columns of each RA/Dec Jacobian. informationObjective expands those
    % selected slices to the legacy 2x6 form only for the small active
    % network, preserving lookup-table speed without storing zero columns.
    selectedJacobians = ...
