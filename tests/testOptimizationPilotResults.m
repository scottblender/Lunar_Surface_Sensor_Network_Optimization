...
%   2. Representative RSO geometry with the optimized network.
...
assert(isgraphics(figureInfo.rsoPopulation,"figure"), ...
    "Representative-RSO figure was not created.");
...
assert(isfile(figureInfo.rsoOutputFile), ...
    "Representative-RSO EPS was not exported.");
assert(isfile(figureInfo.convergenceOutputFile), ...
    "Convergence EPS was not exported.");

assert(~isempty(figureInfo.representativeRsoIndices), ...
    "No representative RSOs were selected for visualization.");
assert(numel(unique(figureInfo.representativeRsoIndices)) == ...
    numel(figureInfo.representativeRsoIndices), ...
    "Representative RSO indices contain duplicates.");
assert(all(figureInfo.representativeRsoIndices >= 1) && ...
    all(figureInfo.representativeRsoIndices <= numberOfObjects), ...
    "Representative RSO indices are outside the validation population.");
...
fprintf("Representative RSOs:     %s\n", ...
    mat2str(figureInfo.representativeRsoIndices(:).'));
